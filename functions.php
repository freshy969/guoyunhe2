 ...
 */
function guoyunhe2_excerpt_more( $link ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	$link = sprintf( '<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>',
		...
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'guoyunhe2' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'guoyunhe2_excerpt_more' );
 ...
 */
function guoyunhe2_colors_css_wrap() {
...
?>
	<style type="text/css" id="custom-theme-colors" <?php if ( is_customize_preview() ) { echo 'data-hue="' . $hue . '"'; } ?>>
		<?php echo guoyunhe2_custom_colors_css(); ?>
	</style>
<?php }
add_action( 'wp_head', 'guoyunhe2_colors_css_wrap' );
...
function guoyunhe2_scripts() {
	...
	wp_enqueue_script( 'guoyunhe2-custom-header', get_theme_file_uri( '/assets/js/custom-header.js' ), array(), '1.0', true );
